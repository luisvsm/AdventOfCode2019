...
function GetValue(mode, intValue) {
    if (mode == Mode.Immediate) {
        ...
        if (memory[intValue] == undefined) {
            console.log("Invalid memory position: " + intValue, "mode: " + mode, memory, pointer);
            return 0;
        } else {
            //console.log("Mode.Position: " + memory[intValue]);
            return memory[intValue];
        }
    } else if (mode == Mode.Relative) {
        if (memory[intValue + relativeBase] == undefined) {
            console.log("Invalid memory position: " + intValue, "mode: " + mode, memory, pointer);
            return 0;
        } else {
            // console.log("Mode.Relative: memory[" + intValue + " + " + relativeBase + "] " + memory[intValue + relativeBase]);
            return memory[intValue + relativeBase];
        }
    } else {
...
function SetValue(mode, setPointer, intValue) {
    if (mode == Mode.Relative) {
        memory[memory[setPointer] + relativeBase] = intValue;
    } else {
        memory[memory[setPointer]] = intValue;
    }
}
...
function Tick() {
    ...
    //console.log("Reading: ", runCode, pointer);
    if (runCode > 20000) {
        runCode -= 20000;
        mode[2] = Mode.Relative;
    } else if (runCode > 10000) {
        runCode -= 10000;
        mode[2] = Mode.Immediate;
    }

    if (runCode > 2000) {
        runCode -= 2000;
        mode[1] = Mode.Relative;
    } else if (runCode > 1000) {
        runCode -= 1000;
    ...
    if (runCode > 200) {
        runCode -= 200;
        mode[0] = Mode.Relative;

    } else if (runCode > 100) {
        runCode -= 100;
        mode[0] = Mode.Immediate;
    }

    //console.log("Reading: ", runCode, mode);

    //console.log("mode: ", mode);
    if (runCode == 1) {
        // Add
        SetValue(mode[2], pointer + 3,
        ...
        if (input === undefined) {
            ...
            SetValue(mode[0], pointer + 1, input);
            input = undefined;
            pointer += 2;
        }
    } else if (runCode == 4) {
        console.log(GetValue(mode[0], memory[pointer + 1]));
        pointer += 2;
    } else if (runCode == 5) {
        ...
        if (GetValue(mode[0], memory[pointer + 1]) < GetValue(mode[1], memory[pointer + 2])) {
            SetValue(mode[2], pointer + 3, 1);
        } else {
            SetValue(mode[2], pointer + 3, 0);
        }
        pointer += 4;
    } else if (runCode == 8) {
        if (GetValue(mode[0], memory[pointer + 1]) == GetValue(mode[1], memory[pointer + 2])) {
        ...
        // console.log(relativeBase + " += " + GetValue(mode[0], memory[pointer + 1]));
        relativeBase += GetValue(mode[0], memory[pointer + 1]);
        pointer += 2;
    } else if (runCode == 99) {
        abort = "Halt";
    } else {
        abort = "Unknown code: " + runCode;
