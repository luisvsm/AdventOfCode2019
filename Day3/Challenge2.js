...
function GetCollisions(wire1Array, wire2Array) {
    ...
    var addWireToBoolMap = (wireArray, addToBoolMap) => {
        ...
        for (let i = 1; i < wireArray.length; i++) {
            ...
            while (currentPost.y < wireArray[i].y) {
                currentPost.y += 1;
                //console.log(currentPost);
                stepCount++;
                addWire();
        ...
        if (addToBoolMap === false) {
            console.log("minPoint", minPoint, minDistance);
            DrawPoint(minPoint, "rgb(0,0,255)");
        }
    }
    addWireToBoolMap(wire1Array, true);
    addWireToBoolMap(wire2Array, false);
}

function GetMaxBounds(wire1Array, wire2Array) {
    minX = wire1Array[0].x;
    minY = wire1Array[0].y;
    maxX = wire1Array[0].x;
    maxY = wire1Array[0].y;
    ...
    var maxBounds = (wireArray) => {
        for (let i = 0; i < wireArray.length; i++) {
            if (wireArray[i].x < minX)
                minX = wireArray[i].x;
            else if (wireArray[i].x > maxX)
                maxX = wireArray[i].x;

            if (wireArray[i].y < minY)
                minY = wireArray[i].y;
            else if (wireArray[i].y > maxY)
                maxY = wireArray[i].y;
        }
    }

    maxBounds(wire1Array);
    maxBounds(wire2Array);
}

function InstructionsToCartesian(wireInstructionArray) {
    coordinateArray = [];
    coordinateArray[0] = { x: 0, y: 0 };

    var lastPos;
    var intVal;
    var direction;

    for (let i = 0; i < wireInstructionArray.length; i++) {
        lastPos = coordinateArray[i];
        intVal = parseInt(wireInstructionArray[i].slice(1), 10);
        direction = wireInstructionArray[i][0];

        if (isNaN(intVal)) {
            console.log("Unable to parse value: ", wireInstructionArray[i]);
        } else if (direction == "R") {
            coordinateArray[i + 1] = { x: lastPos.x + intVal, y: lastPos.y };
        } else if (direction == "D") {
            coordinateArray[i + 1] = { x: lastPos.x, y: lastPos.y + intVal };
        } else if (direction == "U") {
            coordinateArray[i + 1] = { x: lastPos.x, y: lastPos.y - intVal };
        } else if (direction == "L") {
            coordinateArray[i + 1] = { x: lastPos.x - intVal, y: lastPos.y };
        } else {
            console.log("Unknown direction value: ", direction);
        }
    }
...
DistanceToIntersection(w1, w2);

function ResetCanvas() {
    var c = document.getElementById("myCanvas");
    var ctx = c.getContext("2d");
    console.log(Math.max(Math.abs(maxX), Math.abs(minX)) * 2, Math.max(Math.abs(maxY), Math.abs(minY)) * 2);
    c.width = (Math.max(Math.abs(maxX), Math.abs(minX)) * 2) * scalar;
    c.height = (Math.max(Math.abs(maxY), Math.abs(minY)) * 2) * scalar;
    ctx.clearRect(0, 0, c.width, c.height);
    ctx.beginPath();

    ctx.moveTo(-10 + (c.width / 2), c.height / 2);
    ctx.lineTo(10 + (c.width / 2), c.height / 2);
...
function DrawPoint(point, colour) {
    ...
    ctx.beginPath();
    ctx.fillRect(((point.x * scalar) - ((2 * scalar) / 2)) + (c.width / 2), ((point.y * scalar) - ((2 * scalar) / 2)) + (c.height / 2), 2, 2);
    ctx.stroke();

}
function DrawDebugCanvas(path, colour) {
    var c = document.getElementById("myCanvas");
    var ctx = c.getContext("2d");
    ...
    for (let i = 0; i < path.length; i++) {
        ctx.lineTo(((path[i].x * scalar) + (c.width / 2)), ((path[i].y * scalar) + (c.height / 2)));
    }
